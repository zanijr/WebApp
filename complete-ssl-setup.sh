...
# Complete SSL Setup - Fix the SSL configuration after certificate generation
# The certificate was generated successfully, now we need to configure it

set -e

echo "🔒 Completing SSL Setup for Family Chores App"
echo "=============================================="

# Check if certificate exists
if [ ! -f "/etc/letsencrypt/live/family.bananas4life.com/fullchain.pem" ]; then
    echo "❌ SSL certificate not found. Please run the SSL generation first."
    exit 1
fi

echo "✅ SSL certificate found! Completing setup..."
...
sudo mkdir -p ssl
sudo cp /etc/letsencrypt/live/family.bananas4life.com/fullchain.pem ssl/
sudo cp /etc/letsencrypt/live/family.bananas4life.com/privkey.pem ssl/
sudo chown -R zbonham:zbonham ssl
...
# Create SSL docker-compose configuration
echo "🐳 Creating SSL docker-compose configuration..."
...
if curl -k -f https://family.bananas4life.com > /dev/null 2>&1; then
    echo "✅ HTTPS is working!"
else
    echo "⚠️  HTTPS test failed, but services may still be starting..."
    echo "   Try accessing https://family.bananas4life.com in a few minutes"
fi

# Set up automatic certificate renewal
echo "🔄 Setting up automatic certificate renewal..."
(sudo crontab -l 2>/dev/null | grep -v certbot; echo "0 12 * * * /usr/bin/certbot renew --quiet --deploy-hook 'cp /etc/letsencrypt/live/family.bananas4life.com/fullchain.pem /home/zbonham/WebApp/ssl/ && cp /etc/letsencrypt/live/family.bananas4life.com/privkey.pem /home/zbonham/WebApp/ssl/ && chown zbonham:zbonham /home/zbonham/WebApp/ssl/* && docker-compose -f /home/zbonham/WebApp/docker-compose-ssl.yml restart nginx'") | sudo crontab -
...
echo ""
echo "Certificate expires on: 2025-10-03"
echo "Auto-renewal will keep it updated automatically."
echo ""
echo "🎯 Your Family Chores app is now fully secure and production-ready!"
